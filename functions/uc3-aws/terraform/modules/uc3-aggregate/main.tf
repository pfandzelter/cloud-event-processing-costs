...
resource "aws_s3_object" "uc3-package" {
...
resource "aws_lambda_function" "uc3-aggregate" {
  s3_bucket     = aws_s3_bucket.b.bucket
  s3_key        = aws_s3_object.uc3-package.key
  function_name = "${var.function_name}-${var.run_name}"
  role          = aws_iam_role.uc3.arn
  handler       = "uc3.Aggregate"
...
resource "aws_iam_role_policy_attachment" "dynamo" {
  role       = aws_iam_role.uc3.name
  policy_arn = aws_iam_policy.lambda_dynamo.arn
}

resource "aws_iam_role_policy_attachment" "uc3-basic-exec-role" {
  role       = aws_iam_role.uc3.name
...
resource "aws_apigatewayv2_integration" "url" {
  ...
  integration_uri    = aws_lambda_function.uc3-aggregate.invoke_arn
...
resource "aws_lambda_permission" "api_gw" {
  ...
  function_name = aws_lambda_function.uc3-aggregate.function_name
