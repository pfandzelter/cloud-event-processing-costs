...
resource "google_cloudfunctions_function" "function" {
  ...
  environment_variables = {
    ...
    "COLLECTION"           = "${local.collection}-${var.run_name}",
  }

  # one might think that setting trigger_http to false would simply deactivate the trigger, but no! when you set it to false, the trigger is still there, but it is not active. and then prevents the deployment of the pub sub event trigger
  trigger_http = var.use_pubsub ? null : true

  # a super convoluted way of only putting the event_trigger block if we really need it
